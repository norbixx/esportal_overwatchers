...
<head>
    ...
    <title>Esportal OV Panel - Sprawdzone</title>
    ...
    <div id="con-rep">
            <?php
                if(!isset($_COOKIE['auth_token']) && !isset($_COOKIE['auth_login'])){
                    ...
                    if($permission == 1){
                        echo "
                        <h1>Sprawdzone zgłoszenia</h1><br />
                        <a href='addreport.php'><button class='buttonDark'><i class='fa fa-plus-circle'></i> Dodaj zgłoszenia</button></a>
                        <a href='reportdone.php'><button class='buttonDark active'><i class='fa fa-check-circle'></i> Sprawdzone zgłoszenia</button></a>
                    ...
                    if($flag == 1){
                        ...
                        include "connect.php";
                        $countQuery = "select count(*) as count from replays where done=0 and (ban=1 or ban=2)";
                        ...
                        $replayQuery = "select id, link, repuser, report, repadmin, ban, description, replink, done, repdate from replays where done=0 and (ban=1 or ban=2) LIMIT ".($page-1)*$limit.",".$limit;
                        ...
                        if(mysqli_num_rows($replayDoQuery)){
                            while($replayScore = mysqli_fetch_array($replayDoQuery)){
                                ...
                                echo "
                                <tr>
                                    <td><a href='".$replayScore['link']."'>Link</a></td>
                                    <td>".$replayScore['repuser']."</td>
                                    <td>".$replayScore['report']."</td>
                                    <td>".$replayScore['repadmin']."</td>
                                    ";
                                ...
                                echo "
                                    ...
                                    <td><a href='".$replayScore['replink']."'>REPORT LINK</a></td>
                                    <td>".$replayScore['repdate']."</td>
                                    <td><a href='archive.php?id=".$replayScore['id']."'><button class='buttonDark'><i class='fa fa-archive'></i> Archiwizuj</button></a></td>
                                </tr>";
                            }
                    ...
                    if($page > 1)
                        echo "<a href='reportdone.php?page=".($page-1)."'><button class='buttonDark'><i class='fas fa-angle-left'></i></button></a>
                        ";
                    else
                        ...
                        for($k = 1; $k < $site + 1; $k++){
                            ...
                            if($k == $page)
                                echo "<a href='reportdone.php?page=".$k."'><button class='buttonDark active'>".$k."</button></a>";
                            else
                                echo "<a href='reportdone.php?page=".$k."'><button class='buttonDark'>".$k."</button></a>";
                        }
                    ...
                    if($page > $site)    
                        ...
                        echo "
                        <a href='reportdone.php?page=".($page+1)."'><button class='buttonDark'><i class='fas fa-angle-right'></i></button></a>";
