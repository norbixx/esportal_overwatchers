...
<head>
    ...
    <title>Esportal OV Panel - Statystyki</title>
            ...
            <?php
                if(!isset($_COOKIE['auth_token']) && !isset($_COOKIE['auth_login'])){
                    ...
                    if($permission == 1){
                        ...
                        <a href='stats.php'><button class='buttonDark active'><i class='fa fa-table'></i> Statystyki</button></a>
                        ...
                        if($flag == 1){
                            ...
                            <div class='table-margin'>
                                <div class='left'>
                                <h3>Statystyki globalne:</h3><br />
                                <table>
                                    <tr class='first'>
                                        <td>Użytkownik:</td>
                                        <td>Ilość sprawdzonych zgłoszeń:</td>
                                    </tr>
                                ";

                            include "connect.php";

                            $replayQuery = "select u.login as login, u.permission as permission, count(case when r.repadmin = u.login then 1 else null end) as repCount from users as u, replays as r where u.permission = 0 and (r.ban = 1 or r.ban = 2) group by u.login order by repCount desc"; 
                            ...
                            if(mysqli_num_rows($replayDoQuery)){
                                while($replayScore = mysqli_fetch_array($replayDoQuery)){
                                    ...
                                    if($replayScore['permission'] == 0)
                                        echo "<td><img src='img/overwatcher.png' alt='overwatcher' class='icon'>";
                                    if($replayScore['permission'] == 1)
                                        echo "<td><img src='img/admin.png' alt='admin' class='icon'>";
                                    
                                    echo 
                                        "
                                        <span class='accTheme'>".$replayScore['login']."</span></td>
                                        <td>".$replayScore['repCount']."</td>
                                        </tr>";
                                    
                                }
                            }
                            
                            @mysqli_free_result($accDoQuery);
                        ...
                        echo "
                                ...
                                <div class='right'>
                                    <h3>Statystyki miesięczne:</h3><br />
                                    <table>
                                    <tr class='first'>
                                        <td>Użytkownik:</td>
                                        <td>Ilość sprawdzonych zgłoszeń:</td>
                            ...
                            $replayQuery = "select u.login as login, u.permission as permission, count(case when r.repadmin = u.login then 1 else null end) as repCount from users as u, replays as r where u.permission = 0 and (r.ban = 1 or r.ban = 2) and r.repdate like '".$time."%' group by u.login order by repCount desc";
